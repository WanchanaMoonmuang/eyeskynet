...
def main() :
    ...
    od = detect_od(test_img_nobv)
    cd = detect_cd(test_img_nobv)
    od_r = max(find_rad(test_img_nobv,od))
    cd_r = max(find_rad(test_img_nobv,cd))

    out_img = test_img.copy()
    out_img = cv.circle(out_img, find_center(test_img_nobv), od_r, (255, 0, 0), 5)
    out_img = cv.circle(out_img, find_center(test_img_nobv), cd_r, (255, 0, 0), 5)

    plt.imshow(cv.cvtColor(out_img, cv.COLOR_BGR2RGB))

    cdr = float(cd_r)/float(od_r)
    print("This image CDR : ",cdr)
...
def detect_od(img) :
    ...
    img_g = clahe.apply(img_g)
    th, img_hpix = cv.threshold(img_g, 128, 255, cv.THRESH_BINARY)
    if cv.countNonZero(img_hpix) < 10 :
...
def detect_cd (img) :
    ...
    th, img_hpix = cv.threshold(img_b, 128, 255, cv.THRESH_BINARY)
    if cv.countNonZero(img_hpix) < 10 :
        th, img_hpix = cv.threshold(img_b+img_b, 128, 255, cv.THRESH_BINARY)
